...
# 计算新的点位置
def shrink_points(pos, labels, centers):
    new_pos = np.zeros_like(pos)
    for i in range(len(pos)):
        new_pos[i] = (pos[i] + centers[labels[i]]) / 2
    return new_pos
...
def update(i):
    global agents_pos
    labels, centers = kmeans_clustering(agents_pos, n_clusters, i + 1)
    agents_pos = shrink_points(agents_pos, labels, centers)
    scatter.set_offsets(agents_pos)
    centers_scatter.set_offsets(centers)
...
fig, ax = plt.subplots()
scatter = ax.scatter(agents_pos[:, 0], agents_pos[:, 1], c=np.zeros_like(agents_pos[:, 0]), s=25, zorder=3)
centers_scatter = ax.scatter([], [], c='red', s=100, marker='x', zorder=4)

# 创建动画
ani = FuncAnimation(fig, update, frames=iterations, interval=200)

# 保存动画为GIF
ani.save('kmeans_shrink.gif', writer='imagemagick')
